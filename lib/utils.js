...
utils.dateRangesForFiscalYear = function(year){
	
	var start = moment(String(year)+'-7', 'YYYY-M-D');
	var end = moment(String(year+1)+'-6', 'YYYY-M');
...
utils.fiscalYearDateRangeReadable = function(fiscalYear){
	
	var range = utils.dateRangesForFiscalYear(fiscalYear);
	return range[0].format('D MMM YYYY') + ' to ' + range[1].format('D MMM YYYY');
...
utils.fiscalYearToYearReadable = function(fiscalYear){

	return String(fiscalYear) + '-' + String(Number(fiscalYear + 1));
...
utils.quarterReadable = function(year, quarter){
	...
	return 'Q' + String(quarter) + ' ' + String(year) + '-' + String(year+1) + ' (' + utils.quarterDateRangeReadable(year, quarter) + ')';
