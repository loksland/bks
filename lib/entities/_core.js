...
var core = module.exports = function(entityKey, appConfig, config, dataDir, dataTypes){
	...
	self.readFromFile = function(file){
		...
		if (ext === '.yml'){
			...
			var result = self.setFieldDataFromDataObj(data, true);
			if (result.success){
				success = true;
				self.file = file;				
			} else {
	...
	self.setFieldDataFromDataObj = function(dataObj, wipeExistingData){
	...
	self.getDataFromFileName = function(fileName){
		...
		var data = globdule.feed(fileName).to('date').to('leftoversToSlugAndDescription').end();
		var returnData = {};
		if (data){		
