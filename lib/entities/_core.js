...
var core = module.exports = function(entityKey, appConfig, config, dataDir, dataTypes){
	...
	self.writeToFile = function(containingDir, filename, dataObj){
		...
		if (dataObj.shortcode){
			delete dataObj.shortcode;	
		}
	...
	self.readFromFile = function(file){
		...
		if (ext === '.yml'){
			...
			var result = self.setFieldDataFromDataObj(data, true, true, true);
			if (result.success){
				success = true;
				self.file = file;				
			} else {
	...
	self.setFieldDataFromDataObj = function(dataObj, wipeExistingData, validate, shortcodeNotRequired){
		...
		for (var f = 0; f < self.fields.length; f++){
			...
			if (validate || input !== undefined){
				...
				} else if (validate && field.required && field.data.isEmpty() && (!shortcodeNotRequired || field.slug != 'shortcode')){
		...
		if (validate && !self.isDataValid(shortcodeNotRequired)){
	...
	self.isDataValid = function(shortcodeNotRequired){
		
		for (var f = 0; f < self.fields.length; f++){
			var field = self.fields[f];
			if (field.required && field.data.isEmpty() && (!shortcodeNotRequired || field.slug != 'shortcode')){
				return false;
			}
		}		
		return true;
	...
	self.getDataFromFileName = function(fileName){
		...
		var data = globdule.feed(fileName).to('date').end(); //.to('leftoversToSlugAndDescription', {preserveCase: true}).end(); // leftoversToText
		...
		if (data){		
			...
			if (data._leftovers){
				...
				var ttl = data._leftovers;
				ttl = utils.trim(ttl);
				ttl = ttl.replace(/^(-|_)+/, '');
				ttl = ttl.replace(/(-|_)+$/, '');
				
				returnData.title = self.cleanUpTitleFromFileName(ttl); // From leftoversToSlugAndDescription
	...
	self.cleanUpTitleFromFileName = function(fnTitle){
	
		fnTitle = fnTitle.replace(/(-|_)/g, ' ');
		fnTitle = fnTitle.replace(/ +/g, ' ');
		fnTitle = fnTitle.replace(/^ +/, '');
		fnTitle = fnTitle.replace(/ +$/, '');
	
		if (fnTitle.length >= 1){
			fnTitle = fnTitle.charAt(0).toUpperCase() + fnTitle.substr(1).toLowerCase();
		}
