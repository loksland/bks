...
function newEntity(entityKey){
	...
	var entity = entityTypes[entityKey](appConfig, config, dataDir);
	editEntity(entity);
...
function editEntity(entity){
	...
	quiz(entity.fields, true, entity.onFieldWillSave, entity.onFieldDidSave, function(){
		...
		quiz([field], false, null, null, function(){
...
function quiz(fields, showPrompt, onWillSaveCallback, onDidSaveCallback, onQuizCompleteCallback){
	
	if (showPrompt){
		app.styles.green('Enter "?" for input help, "<" to go back or "cancel" to exit').ln();
	}
	
	ask(fields, onWillSaveCallback, onDidSaveCallback, onQuizCompleteCallback, 0, null);
...
function ask(fields, onWillSaveCallback, onDidSaveCallback, onQuizCompleteCallback, fieldIndex, skipSlugs){
	...
	if (field.setting){
		...
		if (result.success){
			return ask(fields, onWillSaveCallback, onDidSaveCallback, onQuizCompleteCallback, fieldIndex + 1, skipSlugs);
		} 
	}
	...
	lastReq.question(question, function(input){
		...
		if (utils.trim(input) === 'cancel'){
			...
			return ask(fields, onWillSaveCallback, onDidSaveCallback, onQuizCompleteCallback, fieldIndex, skipSlugs);
		} else if (utils.trim(input) === '<' ){
			...
			return ask(fields, onWillSaveCallback, onDidSaveCallback, onQuizCompleteCallback, q, skipSlugs);
		}
		
		// On will save
		// field slug, old value, new value
		if (onWillSaveCallback){
			onWillSaveCallback(input, field);
		}
		...
		if (onDidSaveCallback){
			onDidSaveCallback(result.didChange, field);
		}
