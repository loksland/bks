...
function bks(){
	...
	app.cmd('tax', 'Calculate annual tax return', calcTaxReturn);
...
function outputPotentialConflicts(entity, firstrun){
		...
		if (fs.existsSync(filePath) && filePath != entity.file){
			...
			if (firstrun){
				app.styles.red(pad(LEFT_COL_PAD, 'Path ')).print();
				app.styles.red(filePath).ln();
			}
			...
			for (var i = 0; i < files.length; i++){
				app.styles.yellow(pad(LEFT_COL_PAD, 'File ')).print();
				app.styles.yellow(files[i]).ln();				
			}
			app.styles.ln();
		}
...
function quiz(fields, showPrompt, onQuizCompleteCallback, onQuizSkipCallback){
	...
	if (showPrompt){
		if (_scourInProgress){
			app.styles.green('Enter \'?\' for input help, \'<\' to go to previous field, \'<<\' to go to previous row, \':X\' to skip, \':P\' to reload config').ln();
		} else {
			app.styles.green('Enter \'?\' for input help, \'<\' to go back or \':X\' to cancel').ln();
		}
	}
...
function ask(fields, onQuizCompleteCallback, onQuizSkipCallback, fieldIndex, skipSlugs){
	...
	lastReq.question(question, function(input){
		...
		// >> = go back 1, >>> = go back 2 etc.
		var backCount = 0;
		if (input != undefined && String(utils.trim(input)).toUpperCase().substr(0,2) === '<<'){
			backCount = 1;
			var ipt = String(utils.trim(input)).toUpperCase();
			for (var bc = 2; bc < ipt.length; bc++){
				if (ipt.charAt(bc) == '<'){
					backCount++;
				} else {
					break;
		...
		if (input != undefined && String(utils.trim(input)).toUpperCase() === ':X' || String(utils.trim(input)).toUpperCase() === ':P' || backCount > 0){
			if (onQuizSkipCallback) {
				onQuizSkipCallback(String(utils.trim(input)).toUpperCase() === ':P', backCount);
			} else {
				app.prompt();
			}
			return;
...
function newEntityFromDataEntry(index, rowData, statementFile, scourConfigName, scourCallback, skipIndexes, jumpIndexes){
	...
	var table = new AsciiTable(statementBase + statementExt + ': ' + entityKey);
	...
	if (entityKey === 'in'){
		app.styles.yellow(table.toString()).ln();
	} else {
		app.styles.cyan(table.toString()).ln();
	}
	app.styles.ln();
	...
	if (entityKey){		
		...
		if (config.entities[entityKey].scourrules){
			...
			for (var k = 0; k < config.entities[entityKey].scourrules.length; k++){
				...
				if (rule.criteria){
					...
					var match = false;
					var citeriaSummary = '';
					for (var criteriaField in rule.criteria){ 
					
						// if |criteriaA| OR |criteriaB| then match
						if (dataObj[criteriaField] !== undefined){
							
							var criteriaFields;
							if (typeof(rule.criteria[criteriaField]) == 'string'){
								criteriaFields = [rule.criteria[criteriaField]];
							} else {
								criteriaFields = rule.criteria[criteriaField];
							}
							...
							for (var kk = 0; kk < criteriaFields.length; kk++){
								...
								var m;
								if ((m = re.exec(dataObj[criteriaField])) !== null) {
									match = true;
								} else {
									match = false;
									break
		...
		if (result.success && !autoSkip){
			...
			}, function(reload, backCount){
			
				if (backCount > 0){
				
					app.styles.ln();
					app.styles.yellow(pad(LEFT_COL_PAD, 'Back '+String(backCount)+' row'+(backCount>1?'s':'')+'...')).ln();
					app.styles.ln();
					
					newEntityFromDataEntry(Math.max(0,index - backCount), rowData, statementFile, scourConfigName, scourCallback, skipIndexes, jumpIndexes);
					
				} else if (reload){
				
					app.styles.ln();
					app.styles.yellow(pad(LEFT_COL_PAD, 'Reloading user config...')).ln();
					app.styles.ln();
					
					reloadUserConfig();
					
					newEntityFromDataEntry(index, rowData, statementFile, scourConfigName, scourCallback, skipIndexes, jumpIndexes);
...
function calcBAS(req, res){
	...
	for (var i = 0; i < qrtrs.length; i++){
		...
		var key = qrtrs[i].year + '-' + qrtrs[i].quarter;
		if (i === 0){
			field.default = key;
		}
	...
	quiz([field], false, function(){
		...
		var y = Number(val.split('-')[0]);
		var q = Number(val.split('-')[1]);		
...
function processQuarter(year, quarter) {
	...
	var totalSalesAmtIncGST = currency(0);
...
function calcTaxReturn(req, res){
	
	var field = {};
	
	field.slug = 'year';
	field.type = 'choice';
	field.title = 'Select tax return year';
	field.required = true;
	...
	var currentFiscalYear = utils.getCurrentFiscalYear();
	
	for (var i = 0; i < yrs.length; i++){		
	
		var fiscalYear = yrs[i];	
		var rangeReadable = utils.fiscalYearDateRangeReadable(fiscalYear);
		field.typeConfig.choices[fiscalYear] = utils.fiscalYearToYearReadable(fiscalYear) + ' (' + rangeReadable + ')';		
		
		if (fiscalYear = currentFiscalYear - 1){
			field.default = fiscalYear;
		}
...
function processTaxReturn(fiscalYear){
	
	var fiscalYearDateRange = utils.dateRangesForFiscalYear(fiscalYear);
	...
	var outs = getEntitiesInDateRange('out', fiscalYearDateRange[0], fiscalYearDateRange[1]);
	
	var totalDeductibleAmtIncGST = currency(0);
	var totalDeductibleAmtExGST = currency(0);
	var totalDeductibleGST = currency(0);
	
	var deductibleData = [];	
	var nonDeductibleData = [];	
	
	var createHeading = true;
	var headingRow = [];
	headingRow.push('Category');
	var catEntityTotal = 0;
	
	for (var catslug in config.entities.out.cats){
	
		var catTitle = config.entities.out.cats[catslug];		
		var isNonDeductibleCat = catTitle.split('-').join('').toLowerCase().substr(0,13) == 'nondeductible';		
		var dataArr = isNonDeductibleCat ? nonDeductibleData : deductibleData;
		...
		for (var i = 0; i < outs.length; i++){
			...
			if (entity.cat() == catslug){
			
				catEntityTotal++;
			
				var entityData = entity.dataObj();
				var entityArr = [];
				entityArr.push(''); // Leave for category
				
				for (var j = 0; j < entity.fields.length; j++){
					
					var field = entity.fields[j];
		
					if (field.slug != 'shortcode' && !field.setting && field.slug != 'cat'){
			      
						if (createHeading){
							headingRow.push(field.title);
						}

						var val = entityData[field.slug]
						if (field.type == 'bool'){
							val = val ? 1 : 0;				
						} else if (field.type == 'date'){
							val = entity[field.slug + 'Val']().add(12, 'hours').toDate();
						} 
				...
				if (!isNonDeductibleCat){
				
					var deductibleAmountIncGST = entity.deductibleAmountIncGST();
					var deductibleGSTAmount = entity.deductibleGSTAmount();
					var deductibleAmountExGST = entity.deductibleAmountExGST();
					
					entityArr.push(deductibleAmountIncGST.value, deductibleGSTAmount.value, deductibleAmountExGST.value);
				
					totalDeductibleAmtIncGST = totalDeductibleAmtIncGST.add(deductibleAmountIncGST.value);
					totalDeductibleAmtExGST = totalDeductibleAmtExGST.add(deductibleAmountExGST.value);
					totalDeductibleGST = totalDeductibleGST.add(deductibleGSTAmount.value);		
				...
				if (catEntityTotal == 1){ // First entity in cat
					
					if (dataArr.length == 0){
						if (isNonDeductibleCat){
							dataArr.push(headingRow);
						} else {
							dataArr.push(headingRow.concat(['Deductible INC GST', 'Deductible GST', 'Deductible EX GST']));
						}
					}
					dataArr.push([catTitle]);	
					 
				}
				
				dataArr.push(entityArr);
				
			} else if (config.entities.out.cats[entity.cat()] == undefined){
		
				utils.pr(entity.dataObj());
				throw new Error('Entity cat "'+entity.cat()+'" not found in config data');
	...
	deductibleData.push([]);
	deductibleData.push(['Total deductible INC GST', 'Total deductible GST', 'Total deductible EX GST']);
	deductibleData.push([totalDeductibleAmtIncGST, totalDeductibleGST, totalDeductibleAmtExGST])
	
	// .....................................................................................
	
	var ins = getEntitiesInDateRange('in', fiscalYearDateRange[0], fiscalYearDateRange[1]);
	...
	for (var i = 0; i < ins.length; i++){
		...
		for (var j = 0; j < entity.fields.length; j++){
			...
			if (field.slug != 'shortcode' && !field.setting){
		...
		entityArr.push(entity.GSTPortion().value, entity.nonGSTPortion().value);
		
		totalSalesAmtIncGST = totalSalesAmtIncGST.add(entity.amount());
		totalSalesGSTPortion = totalSalesGSTPortion.add(entity.GSTPortion().value);
		totalSalesNonGSTPortion = totalSalesNonGSTPortion.add(entity.nonGSTPortion().value);
		
		if (createHeading){
			createHeading = false;
		}

		if (inDataArr.length == 0){
			inDataArr.push(headingRow.concat(['GST', 'EX GST']));
		}
		
		inDataArr.push(entityArr);
			
	}
	
	inDataArr.push([]);
	inDataArr.push(['Total sales INC GST', 'Total sales GST', 'Total sales EX GST']);
	inDataArr.push([totalSalesAmtIncGST, totalSalesGSTPortion, totalSalesNonGSTPortion])
	...
	// Summary	
	var table = new AsciiTable(utils.fiscalYearToYearReadable(fiscalYear) + ' Summary');
	table.setHeading('Title', 'Ex GST', 'GST', 'Total');
	table.addRow('Income', totalSalesNonGSTPortion.format(), totalSalesGSTPortion.format(), totalSalesAmtIncGST.format());
	table.addRow('Deductions', totalDeductibleAmtExGST.format(), totalDeductibleGST.format(), totalDeductibleAmtIncGST.format());
	...
	// Write
	var excelResult = saveTaxReturnDataExcelDoc(fiscalYear, inDataArr, deductibleData, nonDeductibleData);	
	app.styles.yellow(pad(LEFT_COL_PAD, excelResult.fileExisted ? 'Overwrote ' : 'Created')).print();
	app.styles.cyan(excelResult.destFile).ln();
...
function saveTaxReturnDataExcelDoc(fiscalYear, inData, deductibleData, nonDeductibleData){

	var destFile = dataDir + config.dirNames.io + path.sep + String(fiscalYear) + path.sep + utils.fiscalYearToYearReadable(fiscalYear)+'.xlsx';
	/*
	...
	*/
	...
	wb.Sheets[sheetName] = sheet_from_array_of_arrays(deductibleData);
	
	sheetName = 'not deducted';
	wb.SheetNames.push(sheetName);
	wb.Sheets[sheetName] = sheet_from_array_of_arrays(nonDeductibleData);
	
	sheetName = 'income';
	wb.SheetNames.push(sheetName);
	wb.Sheets[sheetName] = sheet_from_array_of_arrays(inData);
	
	// ws['!cols'] = wscols;
	
	var fileExisted = fs.existsSync(destFile)
	
	XLSX.writeFile(wb, destFile);
	
	return {fileExisted:fileExisted, destFile:destFile};
...
function booleanPrompt(prompt, defaultBool, callbackYay, callbackNay, callbackCancel){ // isSkipPrompt
