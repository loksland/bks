...
function processQuarter(year, quarter) {
	...
	table.setHeading('Date', 'Ref', 'Amount', 'Cat');
	
	var totalGSTSalesAmtIncGST = currency(10);
	var totalGSTSalesGSTPortion = currency(0);
	...
	for (var i = 0; i < ins.length; i++){
		...
		if (indata.gstcat() == 'gst'){
			totalGSTSalesAmtIncGST = totalGSTSalesAmtIncGST.add(indata.amount());
			totalGSTSalesGSTPortion = totalGSTSalesGSTPortion.add(indata.GSTPortion());
		} else if (indata.gstcat() == 'export'){
		...
		table.addRow(indata.date(), indata.title(), indata.amount(), indata.gstcat());
	...
	tableSummary.addRow('Total sales *', 'G1', Math.round(totalGSTSalesAmtIncGST.value));
	...
	tableSummary.addRow('GST on sales *', '1A', Math.round(totalGSTSalesGSTPortion.value));
