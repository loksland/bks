...
function bks(){
	...
	app.cmd('tax', 'Calculate anual tax return', calcTaxReturn);
...
function processTaxReturn(fiscalYear){
	...
	table.addRow('', '', '', '', '', '');
	table.addRow('TOTAL', '', '', totalSalesAmtIncGST.value, totalSalesNonGSTPortion.value, totalSalesGSTPortion.value);
	...
	for (var i = 0; i < outs.length; i++){
		...
		if (out.gst()){ // Only process GST transactions
			...
			var deductibleAmountIncGST = out.deductibleAmountIncGST();
			var deductibleAmountExGST = out.deductibleAmountExGST();
			var deductibleGSTAmount = out.deductibleGSTAmount();
			...
			table.addRow(out.date(), out.title(), out.amount(), out.percentage(), deductibleAmountIncGST, deductibleAmountExGST, deductibleGSTAmount);
			
		}
	}
	table.addRow('', '', '', '', '', '', '');
	table.addRow('TOTAL', '', '', '', totalDeductibleAmtIncGST.value, totalDeductibleAmtExGST.value, totalDeductibleGST.value);
	...
	saveTaxReturnDataExcelDoc(fiscalYear, null, null);
	
}

function saveTaxReturnDataExcelDoc(fiscalYear, inData, outData){
	...
	*/
	...
	sheetName = utils.fiscalYearToYearReadable(fiscalYear) + ' out';
	wb.SheetNames.push(sheetName);
	wb.Sheets[sheetName] = sheet_from_array_of_arrays(outData);
	
	sheetName = utils.fiscalYearToYearReadable(fiscalYear) + ' in';
	wb.SheetNames.push(sheetName);
